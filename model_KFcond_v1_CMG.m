function model_output = model_KFcond_v1_CMG(params, free_choices, forced_choices, rewards, horizon_sequence,right_info,conf)
%     # This model has:
...
%     # note, always use j to refer to condition 2
    dbstop if error;
    G = conf.G; % num of games
    T = conf.T; % num of forced choices
    ...
    decision_noises = [params.dec_noise_h1_13 params.dec_noise_h5_13;...
                       params.dec_noise_h1_22 params.dec_noise_h5_22];
    ...
    for g=1:G  % loop over games
        ...
        A = info_bonuses(horizon_sequence(g));
        sigma_g = decision_noises(info_sequence(g), horizon_sequence(g));
        bias = biases(horizon_sequence(g));

        for t=1:T  % loop over forced-choice trials

            % if the first bandit was chosen
            if (forced_choices(t,g) == 1) 
                % update LR
        ...
        if (free_choices(t,g) == 1) 
            exp_vals(T+1,g) = mu1(t);
            pred_errors(T+1,g) = (rewards(t,g) - exp_vals(t,g));
            alpha(T+1,g) = alpha1(t+1);
            pred_errors_alpha(T+1,g) = alpha1(t+1) * pred_errors(t,g); % confirm that alpha here should be t+1
        else
            exp_vals(T+1,g) = mu2(t);
            pred_errors(T+1,g) = (rewards(t,g) - exp_vals(t,g));
            alpha(T+1,g) = alpha2(t+1);
            pred_errors_alpha(T+1,g) = alpha2(t+1) * pred_errors(t,g);
        end

        % compute difference in values
        dQ = mu2(T+1) - mu1(T+1) + A * right_info(g) + bias;
